...
```{r}
modelo1 <- lm(y1~x1+x2+x3+x1**2+x2**2+(x3)**2+x1*x2+x1*x3+x2*x3, datos)
coef(modelo1)
```
...
Y obtenemos como resultado que $R^2=$ y $R_{aj}^2=$.
...
A    <- matrix(c(0,2,0,0),
               c(0,0,2,0),
               c(0,0,0,1))
linearHypothesis(modelo2, A, d, test = 'F')
```
